...
> 面向项目原作者：对照源码重新理解系统。  
> 技术栈：Angular 19.2 + NestJS Fastify + Prisma PostgreSQL + OpenAI 兼容 AI（默认 DeepSeek）。（2026-08-07 已从 TypeORM/sql.js/Express 迁出；2026-08-25 本地 SQLite → PostgreSQL 17.11）  
...
本地单机、前后端分离：浏览器跑 Angular；本机 NestJS 提供 `/api` 与静态 `/uploads`；业务数据在本机 PostgreSQL；上传文件在 `apps/data/uploads`；AI 由后端按配置的 OpenAI 兼容地址发 HTTP，**Key 不进前端**。
...
| DB | PostgreSQL 17 | `DATABASE_URL`（本机 17.11）；旧 sqlite 不读 |
| 文件 | `apps/data/uploads/` | Excel 抽图等 → `/uploads` |
| AI | `{baseUrl}/v1/chat/completions` | 封装于 `apps/api/src/ai/ai.service.ts` |

端口：API `3000`，Web `5173`。

### 1.3 数据库（Prisma PostgreSQL）

跑在 NestJS 服务端，连接本机 PostgreSQL 17.11（`DATABASE_URL`）。缺该变量时 `ensureDatabaseUrl()` 抛错，不再拼 sqlite 文件。

2026-08-25 起 datasource 为 `postgresql`，migration 已换 PG 基线。旧 `apps/data/assistant.prisma.sqlite` 若仍在仅备份，**不迁业务数据**；空库靠 seed。Compose 加 postgres 推迟到上云。
...
├── apps/data/                        # uploads（及旧 sqlite 备份，gitignore）
...
**心智模型：** 前端薄、页面胖；后端按业务 Module；DB 是本机 PostgreSQL；上传文件在 `apps/data/uploads`；AI 可换兼容厂商。
