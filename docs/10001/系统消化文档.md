...
> 面向项目原作者：对照源码重新理解系统。  
> 技术栈：Angular 19.2 + NestJS + TypeORM + sql.js + OpenAI 兼容 AI（默认 DeepSeek）。  
...
| ORM | TypeORM | Entity → 表；`synchronize: true` |
| DB | sql.js（`type: 'sqljs'`） | **Node 进程内** SQLite；`autoSave` 写盘 |
...
**不是浏览器内嵌、不是离线 PWA。** 跑在 NestJS 服务端，持久化到 `data/assistant.sqlite`（`apps/api/src/common/paths.ts` → `DB_FILE`）。

原因：本机缺 VS C++ 时 `better-sqlite3` 装不上，改用纯 JS 的 sql.js；个人工具数据量小，足够用。配置见 `apps/api/src/app.module.ts`：`type: 'sqljs'`、`location: DB_FILE`、`autoSave: true`。
